...
function normalizeProjectFolders(projects: any[], storageDir: string, existingIndex: Map<string, { projectNumber: number; folderName: string }>) {
  ...
  const normalized = projects.map((project, index) => {
    ...
    const folderName = typeof project?.folderName === 'string' && project.folderName
      ? project.folderName
...
async function persistProjectFolder(storageDir: string, project: any) {
  const folderName = project.folderName || `project-${padProjectNumber(project.projectNumber || 1)}-${slugify(project.name || project.topic || 'project')}`;
  ...
  if (workflowDraft) {
    await fs.writeFile(path.join(root, 'metadata', 'workflow-draft.json'), JSON.stringify(workflowDraft, null, 2), 'utf-8');
  ...
  for (let index = 0; index < assets.length; index += 1) {
    ...
    await fs.writeFile(path.join(root, 'metadata', `scene-${sceneNumber}.json`), JSON.stringify(sceneMetadata, null, 2), 'utf-8');
  ...
  await fs.writeFile(path.join(root, PROJECT_FILENAME), JSON.stringify(projectPayload, null, 2), 'utf-8');
  await fs.writeFile(path.join(root, 'metadata', 'summary.json'), JSON.stringify(summary, null, 2), 'utf-8');
}
...
export async function writeState(state: StudioState, options?: WriteStateOptions): Promise<StudioState> {
  ...
  await fs.writeFile(filePath, JSON.stringify(persistedState, null, 2), 'utf-8');
  return normalized;
}
