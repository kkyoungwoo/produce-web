...
## 핵심 변경
- 프로젝트 저장 구조를 **프로젝트별 물리 폴더 방식**에서 **`studio-state.json` + `projects/<projectId>.json` 분리 JSON 저장 방식**으로 정리했습니다.
- `studio-state.json`은 전역 설정과 프로젝트 인덱스 요약만 저장하고, 각 프로젝트의 전체 상세 데이터는 `projects/<projectId>.json`에 저장합니다.
...
- 초기 `local-data/tubegen-studio/studio-state.json`은 인덱스 중심 상태로 재정리하고, 프로젝트 상세는 `projects/` 아래 개별 JSON으로 분리했습니다.
...
- `projects/<projectId>.json`: 프로젝트 전체 상세 데이터, assets, prompt, scene metadata
