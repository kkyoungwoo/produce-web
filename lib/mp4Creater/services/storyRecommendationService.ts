  ...
  try {
    ...
    const response = await runOpenRouterText({
      ...
      messages: [
        {
          role: 'system',
          content: '당신은 한국어 영상 스토리 기획 보조다. 직관적이고 자연스러운 값만 JSON으로 반환해라. 키는 genre, mood, endingTone, setting, protagonist, conflict 이다. 사용자가 입력한 주제의 핵심 명사, 감정, 갈등, 장소를 반드시 기준으로 삼아 각 값을 추천하고, 주제와 무관한 흔한 템플릿형 추천은 금지한다. 최근 추천을 반복하지 말고 매번 새 디테일로 제안해라.',
        },
        {
          role: 'user',
...
조건:
1) genre, mood, endingTone, setting, protagonist, conflict 6개를 모두 주제 중심으로 맞춘다
2) 사용자가 입력한 주제에서 떠오르는 장소, 인물, 감정, 사건 방향을 직접 반영한다
3) 주제와 동떨어진 범용 표현은 피한다
4) 너무 과하지 않지만 초보자가 바로 쓸 수 있게 짧고 선명하게 쓴다
5) 같은 주제여도 직전 추천과 표현이 겹치지 않게 바꿔줘.`,
        },
      ],
    });
  ...
  try {
    ...
    const response = await runOpenRouterText({
      ...
      messages: [
        {
          role: 'system',
          content: '한국어 영상 제작용 주제 추천 도우미다. 한 줄 주제만 반환하고 설명은 쓰지 마라. 사용자가 입력한 주제의 핵심 정보와 의도를 유지한 채 더 또렷하고 쓰기 좋은 한 줄로 재구성해야 한다. 기존 입력 문장을 그대로 앞뒤로 붙이지 말고, 입력과 무관한 범용 로맨스/감성 표현으로 새지 마라. 최근 추천이나 샘플 표현을 답습하지 말고 매번 새 관점과 새 훅으로 제안하라.',
        },
        {
          role: 'user',
          content: `콘텐츠 유형: ${contentLabel}
...
조건:
1) 입력의 핵심 주제와 의도는 유지하고 표현만 더 좋게 정리
2) 입력에 있는 대상, 행동, 상황, 감정 축을 가능한 한 살린다
3) 입력과 전혀 다른 장르나 상황으로 튀지 않는다
4) 입력이 길면 정보 밀도에 맞춰 조금 더 구체적으로 쓰고, 무음 모드면 화면 전개와 장면 흐름이 떠오르도록 제안해줘.`,
        },
      ],
    });
  ...
  try {
    ...
    const response = await runOpenRouterText({
      ...
      messages: [
        {
          role: 'system',
          content: '한국어 영상 제작 주제 추천 도우미다. JSON만 반환한다. key는 topics, value는 한국어 한 줄 주제 배열이다. 각 항목은 줄바꿈 없는 한 줄 문장으로 작성한다. 사용자의 입력 주제를 중심축으로 유지하고, 그 안에서 표현과 훅만 다르게 변주한다. 입력이 짧으면 간결하게, 입력이 길거나 정보가 많으면 그 밀도에 맞춰 조금 더 길고 구체적인 한 줄 문장으로 확장한다. 기존 입력을 그대로 복붙하지 말고 새 관점으로 변형하며, 서로 결이 겹치지 않게 다양하게 제안한다. 최근 추천 결과나 샘플 표현을 재활용하지 말고 매번 새로 제안한다.',
        },
        {
          role: 'user',
          content: `콘텐츠 유형: ${contentLabel}
...
5) 모든 후보는 사용자 입력과 직접 연결되어야 하며, 입력과 무관한 뜬금없는 설정은 금지
6) 바로 입력창에 넣어 영상 스토리의 출발점으로 쓸 수 있게
7) 무음 모드면 대사보다 장면 흐름, 감정선, 시각적 전개가 잘 떠오르도록 작성
8) 설명문 없이 주제 문장만`,
        },
      ],
    });
