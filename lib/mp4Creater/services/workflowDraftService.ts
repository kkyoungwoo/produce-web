...
export function ensureWorkflowDraft(studioState?: StudioState | null): WorkflowDraft {
  ...
  return {
    ...
    customScriptSettings: {
      expectedDurationMinutes: Math.max(1, Math.min(16, Number(existing.customScriptSettings?.expectedDurationMinutes || 3))),
      speechStyle: existing.customScriptSettings?.speechStyle || 'yo',
