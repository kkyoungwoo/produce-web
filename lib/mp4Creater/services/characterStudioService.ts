...
function defaultRoleLabel(role: CharacterProfile['role'], order: number, name: string) {
  ...
  if (role === 'narrator') return `${name} / 내레이션`;
...
function makeCharacter(options: {
  ...
  const preview = buildPromptPreviewCard({
    ...
    prompt,
    accent: options.role === 'lead' ? '#8b5cf6' : options.role === 'narrator' ? '#0f766e' : '#2563eb',
    kind: 'character',
  });
...
function fallbackCharacters(script: string, selections: StorySelectionState): CharacterProfile[] {
  ...
  supports.forEach((keyword, index) => {
    cast.push(
      makeCharacter({
        name: keyword,
        role: keyword === '앵커' || keyword === '기자' ? 'narrator' : 'support',
  ...
  while (cast.length < castTarget) {
    ...
    cast.push(
      makeCharacter({
        name: `조연 ${nextOrder - 1}`,
        role: 'support',
        roleLabel: `조연 ${nextOrder - 1} / 주인공의 선택을 비추는 상대 인물`,
        script,
        styleHint: selections.mood,
        castOrder: nextOrder,
      })
  ...
  try {
    const raw = await runOpenRouterText({
      ...
      messages: [
        {
          role: 'system',
          content: '한국어 스토리에서 주인공과 조연을 추출하는 도우미다. JSON만 반환한다. 키는 characters 이고, 배열 원소는 name, roleType, roleLabel, styleHint 를 가진다. roleType은 lead, support, narrator 중 하나다.',
        },
        {
          role: 'user',
          content: `콘텐츠 유형: ${options.contentType}\n주인공 후보: ${options.selections.protagonist}\n대본:\n${options.script}\n\n가장 중요한 인물 2~${desiredCastCount(options.script)}명만 뽑아줘. 1명은 주인공 lead로 두고, 나머지는 support 또는 narrator로 정리해줘. roleLabel은 사용자가 카드에서 바로 이해할 수 있는 한국어 역할명으로 써 줘.`,
        },
      ],
    });
...
export function buildStyleRecommendations(
  ...
  return picked.map(([label, accent], index) => {
    ...
    return buildPromptPreviewCard({
      label,
      subtitle: contentType === 'news' ? 'AI 추천 뉴스 화풍' : 'AI 추천 화풍',
      prompt,
      accent,
      kind: 'style',
      sourceMode: 'ai',
...
export function createCharacterCardFromPrompt(options: {
  ...
  const roleLabel = options.roleLabel || options.description || (role === 'lead' ? '주인공 / 직접 추가한 캐릭터' : role === 'narrator' ? '나레이터 / 직접 추가한 캐릭터' : '조연 / 직접 추가한 캐릭터');
  return {
