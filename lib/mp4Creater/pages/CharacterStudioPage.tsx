...
const CharacterStudioPage: React.FC = () => {
  ...
  const handleExtract = async () => {
    if (!draft?.script.trim()) {
      setNotice('먼저 메인 제작 페이지에서 Step 1~3을 저장해 주세요.');
      return;
    }
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="text-xs font-black uppercase tracking-[0.24em] text-blue-600">Character Studio</div>
              <h1 className="mt-2 text-3xl font-black text-slate-900">대본 기준 캐릭터 / 화풍 정리</h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Step 1~3에서 저장한 대본을 기준으로 주인공과 조연을 추출하고, 화풍 추천 카드와 프롬프트 변형 이미지를 선택할 수 있습니다.</p>
            </div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          ...
          <section className="space-y-6">
            ...
            <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <h3 className="mt-2 text-2xl font-black text-slate-900">Step 4 생성 페이지로 이동</h3>
              <p className="mt-2 text-sm leading-6 text-slate-600">선택된 캐릭터와 화풍은 자동 저장됩니다. Step 4 페이지에서 안내 드롭다운을 보고 씬 생성을 시작하면 자동으로 접히고, 다시 열 수도 있습니다.</p>
              <a href={`${basePath}/scene-studio`} className={`mt-4 inline-flex rounded-2xl px-5 py-4 text-base font-black text-white ${selectedCharacters.length && selectedStyle ? 'bg-blue-600 hover:bg-blue-500' : 'pointer-events-none bg-slate-300 text-slate-500'}`}>Step 4 페이지 열기</a>
            </div>
          </section>
        </div>
      </main>
