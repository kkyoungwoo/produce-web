...
2. Step 3에서 선택한 출연자 카드에 TTS API / 보이스 선택 UI가 바로 보이고 저장값이 유지되는지
3. Step 3에서 선택한 출연자 id가 Step 4에 그대로 유지되는지
4. Step 4에서 캐릭터 느낌 카드 클릭 시 화면 상단으로 스크롤되는지
5. 이미 캐릭터 느낌이 저장된 프로젝트를 다시 열면 Step 3 다음 클릭 시 Step 4 출연자별 제작 영역으로 이어지는지
6. Step 4에는 선택된 출연자만 보이는지
7. Step 4 진입 시 기존 이미지가 있는 출연자는 첫 이미지가 자동 선택되는지
8. Step 4 진입 시 이미지가 없는 선택 출연자는 자동으로 첫 후보 생성이 시작되는지
9. Step 4 캐릭터 후보 UI에서 `+` 카드가 첫 칸에 있는지
10. 새 캐릭터 이미지 생성 시 새 카드가 오른쪽에 추가되고, 포인트가 새 카드 쪽으로 이동하는지
11. 좌우 화살표로 캐릭터 후보를 이동하며 선택할 수 있는지
12. Step 5 화풍 선택 시 새로고침처럼 보이는 무한 effect/깜빡임이 없는지
13. Project Gallery에서 프로젝트 이름 hover/focus/click 시 이름 위치에 `이름 변경` affordance가 보이는지
14. Project Gallery의 `썸네일 제작` 버튼이 `thumbnail-studio` 전용 페이지로 이동하는지
15. Thumbnail Studio에서 배경/주인공/썸네일 문구를 수정해 여러 개 생성할 수 있는지
16. Thumbnail Studio에서 최종 선택한 썸네일이 프로젝트 저장소 카드 대표 썸네일로 보이는지
17. 갤러리 진입/상세 진입/뒤로가기에서 브라우저 히스토리가 자연스럽게 동작하는지
