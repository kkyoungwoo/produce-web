      ...
      <div className="mx-auto flex max-w-[1520px] flex-col gap-3 px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="text-xl font-black text-slate-900 sm:text-2xl">
                mp4Creater <span className="text-blue-600">Studio</span>
              </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <NavAction active={activeSection === 'main'} onClick={onGoMain} href={`${basePath}?new=1`}>
              제작
            </NavAction>
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="text-[11px] font-black uppercase tracking-[0.2em] text-blue-600">고정 진행률</div>
              <div className="mt-1 text-sm font-bold text-slate-700">{progressText || `워크플로우 ${safeProgress}%`}</div>
            </div>
            <div className="text-sm font-black text-slate-900">{safeProgress}%</div>
          </div>
          <div className="mt-3 h-2.5 overflow-hidden rounded-full bg-slate-200">
            <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-400 transition-all duration-500" style={{ width: `${safeProgress}%` }} />
          </div>
        </div>
      </div>
    </header>
