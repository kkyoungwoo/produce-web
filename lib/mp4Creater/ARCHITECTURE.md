...
3. Step 3: 프로젝트 프롬프트 확인 → 최종 대본 생성/수정 → 출연자 선택
   ...
   - Step 4로 이동할 때는 현재 선택된 출연자 id를 우선 넘기고, 재추출이 필요한 경우에만 보존 모드로 재수화합니다.
4. Step 4: 캐릭터 느낌 선택 → 선택 출연자별 대표 이미지 확정
   ...
   - 선택된 출연자에게 이미지가 없으면 첫 후보를 자동 생성 시작하고, 이미지가 있으면 첫 이미지를 자동 대표값으로 선택합니다.
...
## 신규 생성 / 첫 진입 복원 축
- `App.tsx`는 새 프로젝트 생성 시 optimistic project와 session navigation cache를 먼저 붙이고, 가능한 경우 실제 프로젝트 저장을 끝낸 뒤 `step-1?projectId=...`로 이동합니다.
- 프로젝트 상세 저장이 잠시 늦더라도 `app/api/local-storage/project/route.ts`는 `projectIndex` 요약 fallback으로 한 번 더 복원합니다.
- `app/[locale]/mp4Creater/loading.tsx`와 `app/[locale]/mp4Creater/step-6/page.tsx`는 skeleton 기반 로딩 화면을 사용해 첫 진입 blank screen을 막습니다.
- URL에 `projectId`가 있으면 `projectNavigationCache` → 갤러리 목록 → local-only 상세 → force sync 순으로 조회합니다.

## 저장 / 복원 계층
- 1차: IndexedDB 캐시
- 2차: `studio-state.json` 인덱스 JSON
- 3차: `projects/<projectId>.json` 상세 JSON
- 보조: `projectNavigationCache` 세션 캐시

핵심 파일
- `lib/mp4Creater/services/localFileApi.ts`: state/project API 래핑, 캐시, storage fallback
- `lib/mp4Creater/services/projectService.ts`: local-first 프로젝트 조회/저장/복제/가져오기
- `app/api/local-storage/project/route.ts`: 프로젝트 상세 JSON 저장, 인덱스 fallback 조회
- `app/api/local-storage/state/route.ts`: studio-state 저장/불러오기
- `app/api/local-storage/config/route.ts`: storageDir 설정 저장
...
- 기본 생성 모드는 `fresh`입니다. 사용자가 비슷한 결과를 직접 요청하지 않는 한, 대본/씬/이미지/영상/자막은 직전 결과를 그대로 답습하지 않도록 중앙 규칙(`lib/mp4Creater/config/creativeVariance.ts`)을 통과합니다.
- continuity가 필요한 값(선택 출연자, 화풍, 콘셉트)은 유지하지만, 샷 구성/조명/팔레트/후킹 포인트/자막 리듬은 새 안을 우선합니다.
...
- `App.tsx`: route step 진입 시 project summary가 상세 draft를 덮지 않도록 `projectNavigationCache → local detail → sync detail → summary` 우선순위로 복원
- `InputSection.tsx`: 콘셉트 변경 초기화, Step 완료 판정, 출연자 선택 보존, Step 3 안내 스크롤, 대본 재추출 시 동일 인물의 생성 이미지/대표 이미지/보이스 설정 재사용
- `Step4Panel.tsx`: 선택 출연자 한정 표시, 첫 이미지 자동 시작/자동 선택, 후보 히스토리 탐색
...
- `config/creativeVariance.ts`: fresh/similar 규칙, 생성 서명, shot/light/palette/subtitle 변주를 중앙 관리
...
- `projectNavigationCache.ts`: step 간 이동 직전 최신 workflowDraft와 캐릭터 이미지 상태를 session cache에 유지해 Step3/4 재진입 시 캐릭터 카드가 summary로 비지 않게 함
