...
3. Step 3: 프롬프트 확인 → 대본 생성/수정 → 출연자 선택 + TTS 확정
   - 대본이 채워진 상태에서 출연자 미선택으로 `다음으로`를 누르면, Step 4로 튀지 않고 출연자 선택 영역으로 스크롤 이동하며 안내합니다.
   - 선택된 출연자 카드에서는 TTS API와 보이스 선택 UI가 바로 보여야 하며, Step 3 완료 판정도 선택 출연자들의 보이스 설정까지 포함합니다.
...
- Step 3 선택 출연자 카드의 TTS API / 보이스 선택 UI와 저장값이 유지되어야 합니다.
